...
unsigned int nStakeTargetSpacing = 2 * 60; // 2 Minute Block Target
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    if (nHeight > PROTO_CHANGE_BLOCK) // Carpediem New PoS Calculations begin here.
    {
    ...
    {
        // Carpediem Original PoS Calculations
...
static const int64 nTargetTimespan = .16 * 24 * 60 * 60; // 4 Hour
static const int64 nTargetNewTimespan = 30 * nStakeTargetSpacing; // 30 * (2 * 60) 1 Hour
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal carpediemcoin miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in carpediemcoin
    bool fStrictPayToScriptHash = true; // Always active in carpediemcoin
    ...
    //  fees are not collected by miners as in carpediemcoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "carpediemcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Carpediemcoin presented on NBC and CNN";
        CTransaction txNew;
        ...
        txNew.strTxComment = "text:carpediemcoin genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("0x862284a4e27113b0ab0fe3c1939a719d1ed8a4e72f4c28a8374ca2d3d9b33a1e"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("carpediemcoinMiner : proof-of-work not meeting target");
...
void static ThreadcarpediemcoinMiner(void* parg);

static bool fGeneratecarpediemcoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void carpediemcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGeneratecarpediemcoins || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGeneratecarpediemcoins) && !fProofOfStake)
                return;
        }
...
void static ThreadcarpediemcoinMiner(void* parg)
{
    ...
    try
    {
        ...
        carpediemcoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadcarpediemcoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadcarpediemcoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadcarpediemcoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void Generatecarpediemcoins(bool fGenerate, CWallet* pwallet)
{
    fGeneratecarpediemcoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGeneratecarpediemcoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d carpediemcoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadcarpediemcoinMiner, pwallet))
                printf("Error: NewThread(ThreadcarpediemcoinMiner) failed\n");
            Sleep(10);
        }
    }
}
